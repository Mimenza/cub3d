...
/*   Updated: 2024/04/18 10:30:32 by anurtiag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_map(t_game *game)
{
	...
	posx = 0;
	posy = 0;
	px_rela = ((game->p->pos.x * (game->window.size->w / 3)) / (game->map.size->w));
	py_rela = ((game->p->pos.y * game->window.size->h / 3) / (game->map.size->h));
	y = 0;
	while ( y < (game->window.size->h / 3))
	{
		x = 0;
		while (x < (game->window.size->w / 3))
		{
			if ((((x - px_rela) * (x - px_rela)) + ((y - py_rela) * (y - py_rela))) <= 20)
			{
				//If para printear circulito
				my_mlx_pixel_put(game, x, y + 300, 0xfa2e0a);
			}
			else if ((posx != ((x * game->map.size->w) / (game->window.size->w / 3))) || (c_y != posy))
			{
				//If para printear lineas grid
				my_mlx_pixel_put(game, x, y + 300, 0x000000);
				posx = ((x * game->map.size->w) / (game->window.size->w / 3));
				posy = ((y * game->map.size->h) / (game->window.size->h / 3));
			}
			else
			{
				...
				if (game->map.grid[posy][posx] == '0')
					my_mlx_pixel_put(game, x, y + 300, 0xFFFFFF);
				else if (game->map.grid[posy][posx] == '1')
				{
					my_mlx_pixel_put(game, x, y + 300, 0x8a8787);
				}
		...
		c_y = ((y * game->map.size->h) / (game->window.size->h / 3));
		y++;
	}
